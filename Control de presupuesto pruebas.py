...
def seccion_analisis_especial_porcentual(
    ...
    with st.expander(f"{nombre_funcion.upper()}"):
        ...
        dif_s_ing = (real_pct - ppt_pct) if ingreso_real != 0 else 0.0
...
def seccion_analisis_por_clasificacion(
    ...
    with st.expander(clasificacion_nombre):
        ...
        ppt_cta_nom["PPT % CTA"] = np.where(ingreso_ppt_sel != 0, (ppt_cta_nom["Neto_A"] / ingreso_ppt_sel) * 100, 0.0)
        ...
        real_cta_nom["REAL % CTA"] = np.where(ingreso_real_sel != 0, (real_cta_nom["Neto_A"] / ingreso_real_sel) * 100, 0.0)
        ...
        # ✅ %INGRESOS = REAL% - PPT%
        df_cla["%Ingresos"] = df_cla["REAL %"] - df_cla["PPT %"]

        # ============================================================
        # ✅ ING_PROY FILTRADO POR PROYECTO (usa tabla PROY_df_proyeccion)
        # ============================================================
        df_proy_tbl = st.session_state.get("PROY_df_proyeccion", None)

        ing_proy_sel = 0.0
        if isinstance(df_proy_tbl, pd.DataFrame) and not df_proy_tbl.empty and {"Proyecto_A", "Neto_A"}.issubset(df_proy_tbl.columns):
            df_proy_tbl = df_proy_tbl.copy()
            df_proy_tbl["Proyecto_A"] = (
                df_proy_tbl["Proyecto_A"].astype(str).str.strip().str.replace(".0", "", regex=False)
            )
            df_proy_tbl["Neto_A"] = pd.to_numeric(df_proy_tbl["Neto_A"], errors="coerce").fillna(0.0)

            if str(proyecto_nombre).strip().upper() == "ESGARI":
                ing_proy_sel = float(df_proy_tbl["Neto_A"].sum() or 0.0)
            else:
                ing_proy_sel = float(df_proy_tbl[df_proy_tbl["Proyecto_A"].isin(proy)]["Neto_A"].sum() or 0.0)
        else:
            # fallback: usa el global si no existe la tabla proyectada
            ing_proy_sel = float(st.session_state.get("PROY_ingreso_pro_fut", 0.0) or 0.0)

        # ✅ RESTANTE a nivel CLASIFICACIÓN
        df_cla["ING_PROY"] = float(ing_proy_sel)
        df_cla["RESTANTE"] = (df_cla["%Ingresos"] / 100.0) * df_cla["ING_PROY"]
        ...
        df_cta["%Ingresos"] = df_cta["REAL %"] - df_cta["PPT %"]
        ...
        ingresos_pct_value_getter = JsCode("""
            function(params){
                if (params.node && params.node.group) {
                    ...
                    var ppt = agg["PPT NOM"] || 0;
                    var real = agg["REAL NOM"] || 0;
                    var ing_ppt = agg["ING_PPT"] || 0;
                    var ing_real = agg["ING_REAL"] || 0;
                    var ppt_pct = (ing_ppt === 0) ? 0 : (ppt / ing_ppt) * 100;
                    var real_pct = (ing_real === 0) ? 0 : (real / ing_real) * 100;
                    return (real_pct - ppt_pct);
                }
        ...
        gridOptions["columnDefs"] = [
            ...
            # ✅ RESTANTE (renglón ya viene; grupo suma)
            {"field": "RESTANTE", "headerName": "RESTANTE", "type": ["numericColumn"], "aggFunc": "sum",
             "valueFormatter": currency_formatter, "cellStyle": {"textAlign": "right"}},

            {"field": "ING_PPT", "hide": True, "aggFunc": "first"},
            {"field": "ING_REAL", "hide": True, "aggFunc": "first"},
            {"field": "ING_PROY", "hide": True, "aggFunc": "first"},
        ...
        grid_key = f"agrid_mix_{clasificacion_nombre}_{'-'.join(proy)}_{meses_key}_{'-'.join(cecos_sel)}"
...
if not st.session_state["logged_in"]:
    ...
    if st.session_state["rol"] in ["admin"] and "ESGARI" in st.session_state["proyectos"]:
        selected = option_menu(
            menu_title=None,
            options=["Tablero", "Vista Dep.","Vista Proy.","Ingresos", "Consulta", "Variaciones", "Proyectos", "Proyección", "Ajustado", "YTD", "Mensual","OH","Departamentos","Proyección OH", "Modificaciones"],
            icons=[
            "clipboard-data",
            "diagram-3-fill",
            "kanban-fill",
        ...
        menu_title=None,
        options=["Tablero", "Vista Dep.","Vista Proy.","Ingresos", "Consulta", "Variaciones", "Proyectos", "Proyección", "Ajustado", "YTD", "Mensual","OH","Departamentos","Proyección OH", "Modificaciones"],
        icons=["clipboard-data", "diagram-3-fill", "kanban-fill", "cash-coin", "search", "arrow-left-right", "kanban", "graph-up-arrow", "bar-chart-line","calendar-range", "calendar-month", "building", "diagram-3", "sliders", "tools"],
        default_index=0,
        orientation="horizontal",)
    ...
    df_pro = df_junto.copy()
    st.session_state["df_pro"] = df_pro
    ...
    if selected == "Tablero":
        ...
        def uo_objetivos_en_tablero(df_ppt, df_real, fecha_actualizacion, proyectos, list_pro, meses_sel):
            ...
            st.markdown('<div class="header-pill">Utilidad Operativa por Proyecto</div>', unsafe_allow_html=True)
            ...
            # ✅ etiquetas visibles
            fig.update_traces(textfont=dict(color="white", size=12, family="Arial Black"))
            ...
            fig.update_layout(
                ...
                plot_bgcolor="white",
                paper_bgcolor="white",
                font=dict(color="#214a6b")
            )
            st.plotly_chart(fig, use_container_width=True, key="fig_uo_objetivos_tablero")

        def resumen_empresa(df_ppt, df_real):
            st.markdown('<div class="header-pill">Resumen Presupuestal – ESGARI</div>', unsafe_allow_html=True)

            # ✅ filtros en la MISMA FILA (menos espacio)
            fc1, fc2 = st.columns([1.5, 1.0], vertical_alignment="center")
            with fc1:
                st.markdown('<span class="filter-pill">MES(ES)</span>', unsafe_allow_html=True)
                meses_ordenados = ["ene.","feb.","mar.","abr.","may.","jun.","jul.","ago.","sep.","oct.","nov.","dic."]
            ...
            with fc2:
                st.markdown('<span class="filter-pill">VISTA</span>', unsafe_allow_html=True)
                vista = st.selectbox(" ", ["Real vs PPT"], index=0, key="vista_empresa", label_visibility="collapsed")
            ...
            with g1:
                ...
                fig = go.Figure()
                fig.add_trace(go.Bar(name="PPT",  x=meses_sel_ord, y=ing_ppt_mes, text=ing_ppt_mes))
                fig.add_trace(go.Bar(name="REAL", x=meses_sel_ord, y=ing_real_mes, text=ing_real_mes))
                ...
                fig.update_traces(
                    ...
                    cliponaxis=False,
                    textfont=dict(color="#214a6b", size=12, family="Arial Black")
                )

                fig.update_layout(
                    barmode="group",
                    height=360,
                    title="Ingresos (PPT vs Real)",
                    plot_bgcolor="white",
                    paper_bgcolor="white",
                    font=dict(color="#214a6b"),
            ...
            with g2:
                st.markdown('<div class="card">', unsafe_allow_html=True)
                difs = [(coss_total_real - coss_total_ppt), (gadm_real - gadm_ppt), (patio_real - patio_ppt)]
                fig2 = go.Figure()
                ...
                fig2.update_layout(
                    height=360,
                    title="Drivers de variación vs PPT",
                    plot_bgcolor="white",
                    paper_bgcolor="white",
                    font=dict(color="#214a6b"),
                    margin=dict(l=20, r=20, t=60, b=20),
            ...
            st.markdown('<div class="header-pill">Resumen Ejecutivo (Empresa)</div>', unsafe_allow_html=True)
            st.dataframe(
                df_res.style.format({
                    "PPT": "${:,.0f}",
                    "REAL": "${:,.0f}",
        ...
        def tabla_proyectos_proyectado(df_ppt, df_real, fecha_actualizacion, proyectos, list_pro):
            ...
            - ING_PROY: histórico ingreso (ratio va/fin)
            - FIJOS: LM (mes anterior) sin variables
            - VARIABLES: mes actual normalizadas y escaladas a ING_PROY
            - PATIO: LM
            """
            ...
            def ingreso_proy_historico(df_base, fecha_actualizacion, mes_act, codigo_list, pro_nombre):
                ...
                idx = (df["mes"] - fecha_act).abs().idxmin()
                dia_ref = int(df.loc[idx, "mes"])

                df_va = df[df["mes"] == dia_ref].copy()
                df_va["ingreso"] = df_va["ingreso"] / dia_ref * fecha_act
                df_va = df_va.drop(columns=["mes", "semana", "fecha"], errors="ignore")
                ...
                df_proy = df_base[df_base["Mes_A"] == mes_act].copy()
                df_proy = _safe_str(df_proy, "Proyecto_A")
                df_proy["Neto_A"] = pd.to_numeric(df_proy["Neto_A"], errors="coerce").fillna(0.0)

                df_proy = df_proy.groupby(["Proyecto_A", "Categoria_A"], as_index=False)["Neto_A"].sum()
                df_proy = df_proy[df_proy["Categoria_A"] == "INGRESO"].drop(columns=["Categoria_A"])

                df_proy["Proyecto_A_num"] = pd.to_numeric(df_proy["Proyecto_A"], errors="coerce")
                df_merged["proyecto_num"] = pd.to_numeric(df_merged["proyecto"], errors="coerce")
                ...
                df_proy["ingreso_dividido"] = pd.to_numeric(df_proy["ingreso_dividido"], errors="coerce")
                mask_ok = df_proy["ingreso_dividido"].notna() & (df_proy["ingreso_dividido"].abs() > 1e-9)

                df_proy.loc[mask_ok, "Neto_A"] = df_proy.loc[mask_ok, "Neto_A"] / df_proy.loc[mask_ok, "ingreso_dividido"]
                df_proy.loc[~mask_ok, "Neto_A"] = 0.0

                if pro_nombre != "ESGARI":
                    cods = [str(x).strip() for x in codigo_list]
                    df_proy = df_proy[df_proy["Proyecto_A"].astype(str).isin(cods)]

                return float(df_proy["Neto_A"].sum() or 0.0)

            def calc_montos_proy(df_base, fecha_actualizacion, mes_act, mes_ant_lm, codigo_list, pro_nombre):
                ingreso_proy = float(ingreso_proy_historico(df_base, fecha_actualizacion, mes_act, codigo_list, pro_nombre) or 0.0)
                if abs(ingreso_proy) < 1e-9:
                    return 0.0, 0.0, 0.0

                df_fix = df_base[df_base["Mes_A"] == mes_ant_lm].copy()
                df_fix = _safe_str(df_fix, "Proyecto_A")
                df_fix["Neto_A"] = pd.to_numeric(df_fix["Neto_A"], errors="coerce").fillna(0.0)
                df_fix = df_fix[~df_fix["Categoria_A"].isin(costos_variables)]
                ...
                patio_proy = float(patio(df_base, [mes_ant_lm], codigo_list, pro_nombre) or 0.0)

                df_junto = pd.concat([df_fix, df_var_proj], ignore_index=True)

                coss_proy = float(df_junto.loc[df_junto["Clasificacion_A"] == "COSS", "Neto_A"].sum() or 0.0) + patio_proy
                gadm_proy = float(df_junto.loc[df_junto["Clasificacion_A"] == "G.ADMN", "Neto_A"].sum() or 0.0)

                return ingreso_proy, coss_proy, gadm_proy
            ...
            mapa = dict(zip(df_visibles["proyectos"], df_visibles["nombre"]))
            proyectos_visibles = df_visibles["proyectos"].tolist()

            df_ppt2 = df_ppt.copy()
            for c in ["Proyecto_A","Mes_A","Clasificacion_A","Categoria_A","CeCo_A"]:
                if c in df_ppt2.columns:
                    df_ppt2[c] = df_ppt2[c].astype(str).str.strip().str.replace(".0", "", regex=False)
            if "Neto_A" in df_ppt2.columns:
                df_ppt2["Neto_A"] = pd.to_numeric(df_ppt2["Neto_A"], errors="coerce").fillna(0.0)

            rows = []
            for p in proyectos_visibles:
                nombre = mapa.get(p, p)
                codigo_list = [_norm_proy_code(p)]

                ing_ppt = float(ingreso(df_ppt2, [mes_act], codigo_list, nombre) or 0.0)
                coss_ppt, _ = coss(df_ppt2, [mes_act], codigo_list, nombre, list_pro)
                coss_ppt = float(coss_ppt or 0.0)
                patio_ppt = float(patio(df_ppt2, [mes_act], codigo_list, nombre) or 0.0)
                coss_total_ppt = coss_ppt + patio_ppt
                gadm_ppt, _ = gadmn(df_ppt2, [mes_act], codigo_list, nombre, list_pro)
                gadm_ppt = float(gadm_ppt or 0.0)

                ing_proy, coss_total_proy, gadm_proy = calc_montos_proy(
                    df_base=df_real,
                    fecha_actualizacion=fecha_actualizacion,
                    mes_act=mes_act,
                    mes_ant_lm=mes_ant_lm,
                ...
                rows.append({
                    "PROYECTO": nombre,
                    "ING_PPT": ing_ppt,
                    "ING_PROY": float(ing_proy),
                    "COSS_PPT": float(coss_total_ppt),
                    "COSS_PROY": float(coss_total_proy),
                    "GADM_PPT": float(gadm_ppt),
                    "GADM_PROY": float(gadm_proy),
                })

            tabla = pd.DataFrame(rows).fillna(0.0)

            tabla["VAR.PPT"] = np.where(tabla["ING_PPT"] != 0, (tabla["ING_PROY"] / tabla["ING_PPT"]) - 1, 0.0)
            tabla["VAR. COSS"] = np.where(tabla["COSS_PPT"] != 0, (tabla["COSS_PROY"] / tabla["COSS_PPT"]) - 1, 0.0)
            tabla["VAR. G.ADM"] = np.where(tabla["GADM_PPT"] != 0, (tabla["GADM_PROY"] / tabla["GADM_PPT"]) - 1, 0.0)
        ...
        df_pro = st.session_state.get("df_pro", None)
        if df_pro is None:
            df_pro = st.session_state.get("PROY_df_junto", None)

        tabla_proyectos_proyectado(
            df_ppt=df_ppt,
            df_real=df_real,
            fecha_actualizacion=fecha_actualizacion,
            proyectos=proyectos,
        ...
        # etiquetas compactas (k / M) para top 10
        def _fmt_compacto(x):
            try:
                x = float(x)
            except:
        ...
        # ✅ CAMBIO: CECO "0" SOLO se agrega cuando el usuario selecciona "Esgari" en el filtro
        # (usamos ceco_nombre porque ahí viene el texto "Esgari")
        esgari_sel = False
        try:
            if isinstance(ceco_nombre, str):
                esgari_sel = (ceco_nombre.strip().lower() == "esgari")
            else:
                esgari_sel = any(str(x).strip().lower() == "esgari" for x in (ceco_nombre or []))
        except:
            esgari_sel = False

        if cecos_sel and esgari_sel and ("0" not in cecos_sel):
            cecos_sel = cecos_sel + ["0"]

        # (opcional recomendado) si NO está Esgari seleccionado y por alguna razón viene "0", lo quitamos
        if cecos_sel and (not esgari_sel) and ("0" in cecos_sel):
            cecos_sel = [x for x in cecos_sel if x != "0"]
        ...
        # 👇 Top 10 por NOMBRE de cuenta
        col_cta_real = _coalesce_cols(df_real, ["Cuenta_Nombre_A", "Cuenta_Nombre", "CuentaNombre", "CuentaNombre_A"])
        ...
        if missing:
            st.error("No encontré columnas necesarias (Mes/CeCo/Neto/Clasificacion/Categoria/Cuenta_Nombre) en df_ppt/df_real.")
            st.stop()
        ...
        dfp[col_cat_ppt] = dfp[col_cat_ppt].astype(str).str.strip()
        dfr[col_cat_real] = dfr[col_cat_real].astype(str).str.strip()
        ...
        dfp_emp = dfp[dfp[col_mes_ppt].isin(meses_usar)].copy()
        dfr_emp = dfr[dfr[col_mes_real].isin(meses_usar)].copy()

        dfp_emp[col_cat_ppt]  = dfp_emp[col_cat_ppt].astype(str).str.strip()
        dfr_emp[col_cat_real] = dfr_emp[col_cat_real].astype(str).str.strip()

        ingreso_ppt_empresa  = _sum_where(dfp_emp, (dfp_emp[col_cat_ppt] == "INGRESO"), col_monto_ppt)
        ingreso_real_empresa = _sum_where(dfr_emp, (dfr_emp[col_cat_real] == "INGRESO"), col_monto_real)
        ...
        total_ppt_ajustado = (_safe_div(total_ppt, ingreso_ppt_empresa) * ingreso_real_empresa) if ingreso_ppt_empresa else total_ppt
        ...
        recomendado_m = (_safe_div(total_ppt, ingreso_ppt_empresa) * ingreso_proyectado) if (ingreso_ppt_empresa and mostrar_recomendado) else 0.0
        pct_ejecutado = (total_real / total_ppt * 100) if total_ppt else 0.0
        pct_ejecutado_ing = (total_real / total_ppt_ajustado * 100) if total_ppt_ajustado else 0.0

        dif_real_vs_ppt = total_real - total_ppt
        dif_s_ingresos = total_real - total_ppt_ajustado
        ...
        with k3:
            st.markdown(f"""
            <div class="kpi">
            <div class="label">TOTAL PPT AJUSTADO</div>
        ...
        st.write("")
        c_pie1, c_pie2, c_difs = st.columns([1.0, 1.0, 0.75])
        ...
        with c_pie2:
            ...
            pie_df2 = pd.DataFrame({
                "Estado": ["Ejecutado", "Restante"],
                "Valor": [max(pct_ejecutado_ing, 0), max(100 - pct_ejecutado_ing, 0)]
            })
            fig2 = px.pie(pie_df2, names="Estado", values="Valor", hole=0.72)
            fig2.update_traces(textinfo="none")
            fig2.update_layout(
                title=f"% Ejecutado sobre Ingresos (Gastado / PPT Ajustado) - {modo}",
                height=330,
                margin=dict(l=10, r=10, t=60, b=10),
                showlegend=True,
                legend=dict(orientation="h", yanchor="bottom", y=-0.1, xanchor="center", x=0.5)
            ...
            st.markdown(
                f"<div style='text-align:center; margin-top:-195px; font-size:34px; font-weight:900;'>{pct_ejecutado_ing:.0f}%</div>",
                unsafe_allow_html=True
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        with c_difs:
            ...
            st.markdown(f"""
            <div class="kpi">
            <div class="label">DIF. Real vs PPT Ajustado</div>
        ...
        with d1:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            df_coss_cat = dfr_f[dfr_f[col_clas_real] == "COSS"].copy()
            df_coss_cat[col_cat_real] = df_coss_cat[col_cat_real].astype(str).str.strip()
            df_coss_cat["Monto"] = pd.to_numeric(df_coss_cat[col_monto_real], errors="coerce").fillna(0)

            grp = df_coss_cat.groupby(col_cat_real, as_index=False)["Monto"].sum()
            grp = grp[grp["Monto"] != 0].copy()

            if grp.empty:
                st.info("No hay datos para participación por categoría (COSS).")
            else:
                fig_coss = px.pie(grp, names=col_cat_real, values="Monto", hole=0.62)
                fig_coss.update_layout(
                    title="Participación del gasto por categoría (COSS) - REAL",
        ...
        with d2:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            df_gadm_cat = dfr_f[dfr_f[col_clas_real] == "G.ADMN"].copy()
            df_gadm_cat[col_cat_real] = df_gadm_cat[col_cat_real].astype(str).str.strip()
            df_gadm_cat["Monto"] = pd.to_numeric(df_gadm_cat[col_monto_real], errors="coerce").fillna(0)

            grp2 = df_gadm_cat.groupby(col_cat_real, as_index=False)["Monto"].sum()
            grp2 = grp2[grp2["Monto"] != 0].copy()

            if grp2.empty:
                st.info("No hay datos para participación por categoría (G.ADMN).")
            else:
                fig_gadm = px.pie(grp2, names=col_cat_real, values="Monto", hole=0.62)
                fig_gadm.update_layout(
                    title="Participación del gasto por categoría (G.ADMN) - REAL",
                    height=320, margin=dict(l=10, r=10, t=60, b=10)
                )
        ...
        def top10_cuentas_por_nombre(df_real_filtrado, clasificacion_obj):
            ...
            # etiqueta compacta
            g["Etiqueta"] = g["Monto"].apply(_fmt_compacto)
            return g

        with t1:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            top_coss = top10_cuentas_por_nombre(dfr_f, "COSS")
            if top_coss.empty:
                st.info("No hay datos REAL para TOP 10 cuentas (COSS).")
            else:
                fig_top_coss = px.bar(
                    top_coss.sort_values("Monto", ascending=True),
                    x="Monto",
                    y=col_cta_real,
                    text="Etiqueta"
                )
                fig_top_coss.update_traces(textposition="outside", cliponaxis=False)
        ...
        with t2:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            top_gadm = top10_cuentas_por_nombre(dfr_f, "G.ADMN")
            if top_gadm.empty:
                st.info("No hay datos REAL para TOP 10 cuentas (G.ADMN).")
            else:
                fig_top_gadm = px.bar(
                    top_gadm.sort_values("Monto", ascending=True),
                    x="Monto",
                    y=col_cta_real,
                    text="Etiqueta"
                )
                fig_top_gadm.update_traces(textposition="outside", cliponaxis=False)
        ...
        def ingreso_proyectado_por_filtro(proys_sel, proyecto_nombre):
            """
            Usa st.session_state["PROY_df_proyeccion"] (de tu bloque Proyección)
            columnas esperadas: Proyecto_A, Neto_A

            Regla:
            - Proyecto "0" solo suma si proyecto_nombre == "ESGARI"
            """
            df_proy = st.session_state.get("PROY_df_proyeccion", None)
            if df_proy is None or getattr(df_proy, "empty", True):
                return 0.0

            df_proy = df_proy.copy()
            if ("Proyecto_A" not in df_proy.columns) or ("Neto_A" not in df_proy.columns):
                return 0.0

            df_proy["Proyecto_A"] = df_proy["Proyecto_A"].astype(str).str.strip().str.replace(".0", "", regex=False)
            df_proy["Neto_A"] = pd.to_numeric(df_proy["Neto_A"], errors="coerce").fillna(0.0)

            # ESGARI: suma todo (incluye "0")
            if str(proyecto_nombre).strip().upper() == "ESGARI":
                return float(df_proy["Neto_A"].sum() or 0.0)

            # No ESGARI: filtra solo proys_sel y excluye "0"
            proys_sel = [str(x).strip().replace(".0", "") for x in (proys_sel or []) if str(x).strip()]
            proys_sel = [p for p in proys_sel if p != "0"]
            if not proys_sel:
                return 0.0

            return float(df_proy[df_proy["Proyecto_A"].isin(proys_sel)]["Neto_A"].sum() or 0.0)
        ...
        esgari_sel = False
        try:
            if isinstance(proyecto_nombre, str):
                esgari_sel = (proyecto_nombre.strip().lower() == "esgari")
            else:
                esgari_sel = any(str(x).strip().lower() == "esgari" for x in (proyecto_nombre or []))
        except:
            esgari_sel = False

        proys_sel = _norm_list(proyecto_codigo)
        if proys_sel and esgari_sel and ("0" not in proys_sel):
            proys_sel = proys_sel + ["0"]
        if proys_sel and (not esgari_sel) and ("0" in proys_sel):
            proys_sel = [x for x in proys_sel if x != "0"]
        col_mes_ppt  = _coalesce_cols(df_ppt,  ["Mes_A"])
        col_mes_real = _coalesce_cols(df_real, ["Mes_A"])
        ...
        if missing:
            st.error("No encontré columnas necesarias (Mes/Proyecto/Neto/Clasificacion/Categoria/Cuenta_Nombre) en df_ppt/df_real.")
            st.stop()
        ...
        # normaliza categoría por si viene con espacios
        dfp[col_cat_ppt] = dfp[col_cat_ppt].astype(str).str.strip()
        dfr[col_cat_real] = dfr[col_cat_real].astype(str).str.strip()
        ...
        ingreso_proyectado = ingreso_proyectado_por_filtro(proys_sel, proyecto_nombre)
        ...
        pct_ejecutado = (total_real / total_ppt * 100) if total_ppt else 0.0
        pct_participacion = (total_real / total_ppt_ajustado * 100) if total_ppt_ajustado else 0.0
        ...
        with c_pie1:
            ...
            fig.update_traces(textinfo="none")
            fig.update_layout(
                title=f"Porcentaje del presupuesto ejecutado - {modo}",
                height=330,
                margin=dict(l=10, r=10, t=60, b=10),
                showlegend=True,
                legend=dict(orientation="h", yanchor="bottom", y=-0.1, xanchor="center", x=0.5)
        ...
        with c_pie2:
            st.markdown('<div class="card">', unsafe_allow_html=True)
            pie_df2 = pd.DataFrame({
                "Estado": ["Participación", "Restante"],
                "Valor": [max(pct_participacion, 0), max(100 - pct_participacion, 0)]
            })
            fig2 = px.pie(pie_df2, names="Estado", values="Valor", hole=0.72)
            fig2.update_traces(textinfo="none")
            fig2.update_layout(
                title=f"% Participación (Gastado / PPT Ajustado) - {modo}",
                height=330,
                margin=dict(l=10, r=10, t=60, b=10),
                showlegend=True,
                legend=dict(orientation="h", yanchor="bottom", y=-0.1, xanchor="center", x=0.5)
            ...
            st.plotly_chart(fig2, use_container_width=True)
            st.markdown(
                f"<div style='text-align:center; margin-top:-195px; font-size:34px; font-weight:900;'>{pct_participacion:.0f}%</div>",
                unsafe_allow_html=True
            )
            st.markdown("</div>", unsafe_allow_html=True)
        ...
        df_ing_bar = pd.DataFrame({
            "Tipo": ["Ingreso Presupuestado", "Ingreso Real", "Ingreso Proyectado"],
            "Monto": [ingreso_ppt, ingreso_real, ingreso_proyectado]
        })

        fig_ing = px.bar(df_ing_bar, x="Tipo", y="Monto", text="Monto")
        fig_ing.update_traces(texttemplate="%{text:,.0f}", textposition="outside", cliponaxis=False)
        fig_ing.update_layout(
            title=f"Ingreso del proyecto ({modo})",
        ...
        with t1:
            ...
            if top_coss.empty:
                ...
                top_coss["Etiqueta"] = top_coss["Monto"].apply(_fmt_compacto)  # ✅ solo número
                fig_top_coss = px.bar(
                    top_coss.sort_values("Monto", ascending=True),
                    x="Monto",
                    y=col_cta_real,
        ...
        with t2:
            ...
            if top_gadm.empty:
                ...
                top_gadm["Etiqueta"] = top_gadm["Monto"].apply(_fmt_compacto)  # ✅ solo número
                fig_top_gadm = px.bar(
                    top_gadm.sort_values("Monto", ascending=True),
                    x="Monto",
                    y=col_cta_real,
        ...
        def tabla_proyectos(df_ppt, df_real, meses_seleccionado, df_proyectos, proyectos_filtrados=None):
            ...
            tabla = tabla[[
                "PROYECTO",
                "COSS PPT (informativo)", "COSS S/INGRESO", "COSS REAL", "DIF. COSS",
                "G.ADMN PPT (informativo)", "G.ADM S/INGRESO", "G.ADM REAL", "DIF. G.ADM",
            ]]

            # --- total
            total = pd.DataFrame([{
                "PROYECTO": "TOTAL",
                "COSS PPT (informativo)": float(tabla["COSS PPT (informativo)"].sum()),
                "COSS S/INGRESO": float(tabla["COSS S/INGRESO"].sum()),
                "COSS REAL": float(tabla["COSS REAL"].sum()),
                "DIF. COSS": float(tabla["DIF. COSS"].sum()),
                "G.ADMN PPT (informativo)": float(tabla["G.ADMN PPT (informativo)"].sum()),
        ...
        def tabla_ceco_ytd_resumen(
            ...
            MESES_ORD = ["ene.", "feb.", "mar.", "abr.", "may.", "jun.", "jul.", "ago.", "sep.", "oct.", "nov.", "dic."]
            idx_mes = {m: i for i, m in enumerate(MESES_ORD)}
            ...
            # -------- filtros
            proy_sel = [norm_proy(p) for p in (proyecto_codigo or []) if str(p).strip()]
            ceco_sel = [norm_ceco(c) for c in (ceco_codigo or []) if str(c).strip()]

            # -------- catálogo cecos (para nombre)
            df_cecos = cargar_datos(cecos_url)
            df_cecos["ceco"] = df_cecos["ceco"].astype(str).str.strip()
            df_cecos["nombre"] = df_cecos["nombre"].astype(str).str.strip()
            ceco_map = dict(zip(df_cecos["ceco"], df_cecos["nombre"]))

            def _prep(df):
                d = df.copy()
                d["Mes_A"] = d["Mes_A"].astype(str).str.strip().apply(norm_mes)
                d["Proyecto_A"] = d["Proyecto_A"].astype(str).str.strip().str.replace(".0", "", regex=False)
                d["CeCo_A"] = d["CeCo_A"].astype(str).str.strip().str.replace(".0", "", regex=False)
                d["Categoria_A"] = d["Categoria_A"].astype(str).str.strip().apply(norm_cat)
                d["Clasificacion_A"] = d["Clasificacion_A"].astype(str).str.strip().apply(norm_clas)
                d["Neto_A"] = pd.to_numeric(d["Neto_A"], errors="coerce").fillna(0.0)
                return d

            ppt = _prep(df_ppt)
            real = _prep(df_real)

            ppt = ppt[ppt["Mes_A"].isin(meses_sel)]
            real = real[real["Mes_A"].isin(meses_sel)]

            if proy_sel:
                ppt = ppt[ppt["Proyecto_A"].isin(proy_sel)]
                real = real[real["Proyecto_A"].isin(proy_sel)]

            if ceco_sel:
                ppt = ppt[ppt["CeCo_A"].isin(ceco_sel)]
                real = real[real["CeCo_A"].isin(ceco_sel)]

            ppt_all = _prep(df_ppt)
            real_all = _prep(df_real)
            ppt_all = ppt_all[ppt_all["Mes_A"].isin(meses_sel)]
            real_all = real_all[real_all["Mes_A"].isin(meses_sel)]
            if proy_sel:
                ppt_all = ppt_all[ppt_all["Proyecto_A"].isin(proy_sel)]
                real_all = real_all[real_all["Proyecto_A"].isin(proy_sel)]

            ing_ppt_total = float(ppt_all.loc[ppt_all["Categoria_A"] == "INGRESO", "Neto_A"].sum() or 0.0)
            ing_real_total = float(real_all.loc[real_all["Categoria_A"] == "INGRESO", "Neto_A"].sum() or 0.0)

            # -------- totales por CECO y CLASIFICACION (solo COSS y G.ADMN)
            CLAS = ["COSS", "G.ADMN"]
            ppt_g = ppt[ppt["Clasificacion_A"].isin(CLAS)].copy()
            real_g = real[real["Clasificacion_A"].isin(CLAS)].copy()

            ppt_ceco = (
                ppt_g.groupby(["CeCo_A", "Clasificacion_A"], as_index=False)["Neto_A"]
                .sum()
                .rename(columns={"Neto_A": "PPT"})
            )
            real_ceco = (
                real_g.groupby(["CeCo_A", "Clasificacion_A"], as_index=False)["Neto_A"]
                .sum()
                .rename(columns={"Neto_A": "REAL"})
            )

            t = ppt_ceco.merge(real_ceco, on=["CeCo_A", "Clasificacion_A"], how="outer").fillna(0.0)
            wide_ppt = t.pivot_table(index="CeCo_A", columns="Clasificacion_A", values="PPT", aggfunc="sum", fill_value=0.0)
            wide_real = t.pivot_table(index="CeCo_A", columns="Clasificacion_A", values="REAL", aggfunc="sum", fill_value=0.0)

            out1 = pd.DataFrame(index=sorted(set(list(wide_ppt.index) + list(wide_real.index))))
            out1.index.name = "CeCo_A"

            out1["COSS PPT"] = wide_ppt["COSS"] if "COSS" in wide_ppt.columns else 0.0
            out1["COSS REAL"] = wide_real["COSS"] if "COSS" in wide_real.columns else 0.0
            out1["% ACUMULADO COSS"] = np.where(out1["COSS PPT"] != 0, (out1["COSS REAL"] / out1["COSS PPT"]) * 100.0, 0.0)

            out1["GADMN PPT"] = wide_ppt["G.ADMN"] if "G.ADMN" in wide_ppt.columns else 0.0
            out1["GADMN REAL"] = wide_real["G.ADMN"] if "G.ADMN" in wide_real.columns else 0.0
            out1["% ACUMULADO GADMN"] = np.where(out1["GADMN PPT"] != 0, (out1["GADMN REAL"] / out1["GADMN PPT"]) * 100.0, 0.0)

            out1 = out1.reset_index()
            out1["CeCo"] = out1["CeCo_A"].map(ceco_map).fillna(out1["CeCo_A"])
            out1 = out1.drop(columns=["CeCo_A"])
            out1 = out1.sort_values("% ACUMULADO COSS", ascending=False)

            # ===== TOTAL TABLA 1 =====
            total1 = {
                "CeCo": "TOTAL",
                "COSS PPT": float(out1["COSS PPT"].sum()),
                "COSS REAL": float(out1["COSS REAL"].sum()),
                "% ACUMULADO COSS": (float(out1["COSS REAL"].sum()) / float(out1["COSS PPT"].sum()) * 100.0) if float(out1["COSS PPT"].sum()) != 0 else 0.0,
                "GADMN PPT": float(out1["GADMN PPT"].sum()),
                "GADMN REAL": float(out1["GADMN REAL"].sum()),
                "% ACUMULADO GADMN": (float(out1["GADMN REAL"].sum()) / float(out1["GADMN PPT"].sum()) * 100.0) if float(out1["GADMN PPT"].sum()) != 0 else 0.0,
            }
            out1 = pd.concat([out1, pd.DataFrame([total1])], ignore_index=True)

            def style_acum_1(row):
                styles = [""] * len(row)
                cols = list(row.index)
                for colname in ["% ACUMULADO COSS", "% ACUMULADO GADMN"]:
                    if colname in cols:
                        j = cols.index(colname)
                        v = float(row[colname]) if pd.notnull(row[colname]) else 0.0
                        styles[j] = "background-color:#FF0000;color:white;font-weight:800;" if v > 100 else "background-color:#92D050;color:black;font-weight:800;"
                # resalta total
                if str(row.get("CeCo", "")).upper() == "TOTAL":
                    styles = ["background-color:#FFFFFF; color:black; font-weight:900;"] * len(row)
                return styles

            st.dataframe(
                out1.set_index("CeCo").style
                    .apply(style_acum_1, axis=1)
                    .format({
                        "COSS PPT": "${:,.0f}",
                        "COSS REAL": "${:,.0f}",
                        "% ACUMULADO COSS": "{:.2f}%",
            ...
            out2 = out1[out1["CeCo"] != "TOTAL"].copy()
            out2["ING_PPT_TOTAL"] = float(ing_ppt_total or 0.0)
            out2["ING_REAL_TOTAL"] = float(ing_real_total or 0.0)

            out2["COSS S/INGRESO"] = np.where(
                out2["ING_PPT_TOTAL"] != 0,
                (out2["COSS PPT"] / out2["ING_PPT_TOTAL"]) * out2["ING_REAL_TOTAL"],
                0.0
            )
            out2["% ACUMULADO COSS"] = np.where(
                out2["COSS S/INGRESO"] != 0,
                (out2["COSS REAL"] / out2["COSS S/INGRESO"]) * 100.0,
                0.0
            )

            out2["GADMN S/INGRESO"] = np.where(
                out2["ING_PPT_TOTAL"] != 0,
                (out2["GADMN PPT"] / out2["ING_PPT_TOTAL"]) * out2["ING_REAL_TOTAL"],
                0.0
            )
            out2["% ACUMULADO GADMN"] = np.where(
                out2["GADMN S/INGRESO"] != 0,
                (out2["GADMN REAL"] / out2["GADMN S/INGRESO"]) * 100.0,
                0.0
            )

            out2 = out2[[
                "CeCo",
                "COSS S/INGRESO", "COSS REAL", "% ACUMULADO COSS",
                "GADMN S/INGRESO", "GADMN REAL", "% ACUMULADO GADMN",
            ]].copy()

            out2 = out2.sort_values("% ACUMULADO COSS", ascending=False)

            # ===== TOTAL TABLA 2 =====
            sum_coss_sing = float(out2["COSS S/INGRESO"].sum())
            sum_coss_real = float(out2["COSS REAL"].sum())
            sum_gadmn_sing = float(out2["GADMN S/INGRESO"].sum())
            sum_gadmn_real = float(out2["GADMN REAL"].sum())

            total2 = {
                "CeCo": "TOTAL",
                "COSS S/INGRESO": sum_coss_sing,
                "COSS REAL": sum_coss_real,
                "% ACUMULADO COSS": (sum_coss_real / sum_coss_sing * 100.0) if sum_coss_sing != 0 else 0.0,
                "GADMN S/INGRESO": sum_gadmn_sing,
                "GADMN REAL": sum_gadmn_real,
                "% ACUMULADO GADMN": (sum_gadmn_real / sum_gadmn_sing * 100.0) if sum_gadmn_sing != 0 else 0.0,
            }
            out2 = pd.concat([out2, pd.DataFrame([total2])], ignore_index=True)

            def style_acum_2(row):
                styles = [""] * len(row)
                cols = list(row.index)
                for colname in ["% ACUMULADO COSS", "% ACUMULADO GADMN"]:
                    if colname in cols:
            ...
            st.dataframe(
                out2.set_index("CeCo").style
                    .apply(style_acum_2, axis=1)
                    .format({
                        "COSS S/INGRESO": "${:,.0f}",
                        "COSS REAL": "${:,.0f}",
                        "% ACUMULADO COSS": "{:.2f}%",
            ...
            st.markdown("### Gráfica — % Acumulado vs s/Ingreso (YTD)")
            df_plot = out2[out2["CeCo"] != "TOTAL"].copy()
            df_plot["CeCo"] = df_plot["CeCo"].astype(str)
            TOP_N = 20
            df_plot = df_plot.sort_values("% ACUMULADO COSS", ascending=False).head(TOP_N)

            fig = go.Figure()
            fig.add_trace(
                go.Scatter(
                    x=df_plot["CeCo"],
                    y=df_plot["% ACUMULADO COSS"],
                    mode="lines+markers",
                    name="COSS % Acumulado",
                    hovertemplate="<b>%{x}</b><br>COSS: %{y:.2f}%<extra></extra>",
                )
            )
            ...
            fig.add_trace(
                go.Scatter(
                    x=df_plot["CeCo"],
                    y=df_plot["% ACUMULADO GADMN"],
                    mode="lines+markers",
                    name="G.ADMN % Acumulado",
                    hovertemplate="<b>%{x}</b><br>G.ADMN: %{y:.2f}%<extra></extra>",
                )
            )
        ...
        if meses_seleccionado:
            _t1, _t2 = tabla_ceco_ytd_resumen(
                df_ppt=df_ppt,
                df_real=df_real,
                meses_seleccionado=meses_seleccionado,
                proyecto_codigo=proyecto_codigo,
                proyecto_nombre=proyecto_nombre,
                ceco_codigo=ceco_codigo,
        ...
        def tabla_real_vs_ppt_por_ceco(df_ppt, df_real, meses_seleccionado, proyectos_filtrados, df_cecos):
            ...
            # ====== CAMBIO ÚNICO: NOMBRES INFORMATIVOS ======
            out = out.rename(columns={
                "COSS PPT": "COSS PPT (INFORMATIVO)",
                "GADM PPT": "G.ADMN PPT (INFORMATIVO)",
            })
            ...
            total = pd.DataFrame([{
                ...
                "G.ADMN PPT (INFORMATIVO)": float(out["G.ADMN PPT (INFORMATIVO)"].sum()),
                "GADM S/INGRESOS": float(out["GADM S/INGRESOS"].sum()),
                "GADM_REAL": float(out["GADM_REAL"].sum()),
                "DIF. GADM": float(out["DIF. GADM"].sum()),

                "COSS PPT (INFORMATIVO)": float(out["COSS PPT (INFORMATIVO)"].sum()),
            ...
            def _pinta_difs_color(df):
                ...
                sty = df.style.format({
                    "G.ADMN PPT (INFORMATIVO)": "${:,.2f}",
                    "GADM S/INGRESOS": "${:,.2f}",
                    "GADM_REAL": "${:,.2f}",
                    "DIF. GADM": "${:,.2f}",

                    "COSS PPT (INFORMATIVO)": "${:,.2f}",
                    "COSS S/INGRESO": "${:,.2f}",
                    "COSS_REAL": "${:,.2f}",
                    "DIF COSS": "${:,.2f}",
                })
        ...
        with tab_actual:
            ...
            ppt_ceco = (
                df_ppt_oh
                .groupby("CeCo_A", as_index=False)["Neto_A"]
                .sum()
                .rename(columns={"Neto_A": "OH_PPT"})
            )
        ...
        def tabla_diferencias(df_ppt: pd.DataFrame, df_base: pd.DataFrame):
            ...
            tot_nuevos = float(out_final.loc[out_final["TIPO_CAMBIO"] == "NUEVO", "Neto_A_PPT"].sum())
            tot_elim   = float(out_final.loc[out_final["TIPO_CAMBIO"] == "ELIMINADO", "Neto_A_BASE"].sum())
            tot_dif    = float(out_final["DIF_NETO"].sum())

            resumen = pd.DataFrame([{
                "NUEVOS (MXN)": tot_nuevos,
                "ELIMINADOS (MXN)": tot_elim,
                "DIF TOTAL (MXN)": tot_dif,
                "REGISTROS": int(len(out_final)),
                "NUEVOS #": int((out_final["TIPO_CAMBIO"] == "NUEVO").sum()),
                "ELIMINADOS #": int((out_final["TIPO_CAMBIO"] == "ELIMINADO").sum()),
            }])
